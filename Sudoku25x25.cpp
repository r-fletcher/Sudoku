...
for (int i = 0; i < N; i++)
    for (int j = 0; j < N; j++){
        char num;
        cin >> num;
        sudoku[i][j] = char_to_int(toupper(num));
    }
...
void set_initial_masks(vector<vector<int>> &sudoku, vector<vector<int>> &possible_values){
    for (int i = 0; i < N; i++){
        for (int j = 0; j < N; j++){
            if (sudoku[i][j] == 0){
                possible_values[i][j] = 0b1111111111111111111111111; //any number is possible
            }
            else {
                possible_values[i][j] = 0;
            }
...
void output(vector<vector<int>> &sudoku) {
    for (int i = 0; i < N; i ++){
        for (int j = 0; j < N; j ++){
            if (j == N-1)
                cout << int_to_char(sudoku[i][j]) << endl;
            else
                cout << int_to_char(sudoku[i][j]) << " ";
        }
    }
}
